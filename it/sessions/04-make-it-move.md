...
recover: Se tutti e otto gli hi-hat si sono abbassati, non era tenuto niente e il giro ha mosso la VEL di tutta la subtrack: rialzala, tieni premuti i quattro tasti, poi gira di nuovo. NOTE mostra un trattino: il §12.2 dice che non è disponibile su una traccia Subtracks.
:::

:::note
Un lock che metti qui appartiene alla subtrack in cui sei e a nient'altro: ogni subtrack
tiene le proprie impostazioni SRC, FLTR, AMP e MOD (§A.2.3), quindi questo giro lascia il
...
recover: Se i due suonano identici, il lock non è mai arrivato — torna indietro di un passo, tieni premuti i quattro tasti e guarda VEL diventare invertito mentre giri la manopola.
:::
...
recover: Se tutti gli hi-hat sono diventati opachi insieme, [TRIG 9] non era premuto quando hai girato: prima tienilo premuto.
:::
...
recover: Se l'hi-hat è sparito del tutto, quella era una pressione breve e non una tenuta, e toglie il trig — premi [TRIG 9] ancora una volta per riscriverlo, e ogni lock che portava se n'è andato con lui (§10.10.1).
:::
...
ripetizioni crescono invece di spegnersi.
...
keys16: { 5: red, 13: red }
hear: Lo snare sul quarto quarto arriva come un roll breve: le ripetizioni durano quanto la LEN del trig e si fermano dentro il passo 13, un pickup verso la battuta successiva più che un fill attraverso di essa. Lo snare sul 5 è intatto, e sulla pagina i tre che hai girato sono quelli invertiti. Il passo 13 adesso lampeggia, il 5 è fisso.
recover: Un roll che non senti di solito è RTRG ancora spento — tieni premuto [TRIG 13] e guarda quella lettura cambiare prima di avvicinarti a RTIM. Se fanno il roll entrambi gli snare, [TRIG 13] è stato lasciato prima di girare e l'impostazione è andata su tutta la subtrack invece che su quel solo trig: rispegni RTRG senza niente premuto, poi ricomincia tenendo il tasto. Troppo fitto? [F] di nuovo a 1/16 dà una ripetizione per passo, 1/12 dà le terzine.
:::
...
roll è RVEL, e lavora rispetto alla VEL del trig stesso. E il manuale nomina due manopole per
RTRG: la regola del §6.3 è che un parametro risponde alla manopola sopra cui sta, il che mette
RTRG su [E], mentre la frase del §12.2 nomina la manopola [A]. Gira la manopola sotto il
parametro.
:::
...
hear: Il clap sul 7 cade un pelo dopo il passo su cui sta, e la battuta si sporge più forte verso la successiva.
...
per registrare (§10.4); lascialo spento, perché il passo successivo quantizza a mano.
Premuto una volta che [RECORD] è stato lasciato, esce dalla registrazione dal vivo e il pattern continua a girare.
:::
...
ogni traccia del pattern insieme; lasciala a zero. [NO] chiude il menu.

:::checkpoint
screen: { menu: "QUANTIZE", items: [TRK 0, PATTERN 0] }
hear: Entrambe le impostazioni partono da zero. Il kick e lo snare non si muovono, perché sono stati scritti sulla griglia e ci sono già. Quello che TRK tira è tutto ciò che non lo è: la parte che hai suonato a mano, e il clap che hai spinto in ritardo due passi fa (§10.8).
recover: Il menu non si è aperto? Prova invece [FUNC] + [TRIG], la combinazione che il §10.4 dà per questo stesso menu dove il §10.8 dà [FUNC] + [KEYBOARD G#1]. Sei andato fino in fondo e hai perso il clap in ritardo? Torna alla subtrack del clap, tieni premuto [TRIG 7] e premi [RIGHT] per spingerlo di nuovo fuori (§10.5); quantizzare arrotonda al passo più vicino, e per un trig con il micro timing quello non è per forza il passo su cui è stato scritto (§10.8). Se la take ancora non suona come quello che hai suonato, [un proprietario riporta lo stesso](https://www.elektronauts.com/t/tonverk-user-thread/238631/1812), senza risposta nel thread.
:::
...
Poi tieni premuto [TRK] e premi [KEYBOARD D1], il secondo tasto SUBTRACKS, e guarda lo
...
La sessione 5 fa smettere il pattern di ripetersi: condizioni di trig sul kit che hai, un
...
in quella pagina mostra perché un trig non ha suonato.
