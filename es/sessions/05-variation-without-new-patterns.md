...
recover: Si el pickup llega en cada loop, COND sigue en el guion: mantén pulsado [TRIG 15] y mira cómo cambia esa lectura mientras giras [H], en vez de mirar las teclas. Si no llega nunca, te has pasado de 1:2 — las condiciones son una sola lista en un solo mando, y 2:2 suena exactamente en los loops que 1:2 se salta.
:::
...
recover: Si el 16 suena en cada loop, COND sigue en el guion: mantén pulsado [TRIG 16] y gira [H] clic a clic, pasando el par de FILL, hasta que PRE sea la lectura. Y si te tienta apilar un segundo PRE encima de este, seguiría preguntando por el 15 — un trig PRE se salta cuando un PRE posterior mira hacia atrás (§10.10.2). El §10.10.2 describe PRE en una pista, no en un kit: no dice si el 16 mira hacia atrás al trig anterior en esta subtrack o en cualquier parte de la pista 1.
:::

:::note
Una ghost note que pierde su tirada cuenta, en lo que al 16 respecta, como una ghost note que
nunca estuvo ahí. Antes del OS 1.4.0 no era así: las notas de lanzamiento de Elektron
incluyen [un lock de PROB que quedaba fuera del cálculo de PRE y
NEI](https://www.elektron.se/release-notes/tonverk-os-release-notes) entre los errores que la
1.4.0 corrigió, y en un OS más antiguo la pareja se descompone.
:::
...
recover: El simulador cuenta un trig liso, sin condición ni probabilidad, como uno al que PRE puede mirar hacia atrás, tal como se lee la frase del §10.10.2; los propietarios de otras cajas de Elektron con esta misma regla dicen que un trig a secas no se evalúa nunca.
:::
...
recover: Si la carrera suena sin nada pulsado, o los tres trigs siguen en el guion en vez de en FILL, o LATCHING es la opción elegida y un toque anterior dejó el modo FILL activado — pulsa [FILL] una vez y escucha otra vez.
:::
...
recover: Si no cambió nada, el [YES] no cayó en la línea resaltada: abre el menú otra vez y mira qué acción está marcada antes de pulsar. Si la recuperación no devuelve las velocities, el guardado del paso 1 todavía las tiene.
:::
...
su 1:2 y los tres snares nuevos conservan su FILL. Los propietarios confirman que las ocho
subtracks van juntas: han pedido una manera de apuntar una transformación a una sola subtrack y
...
junto con las notas. VELOCITY UNLOCK es un martillo más pesado de lo que sugiere su nombre: quita todos los locks
...
Mantén pulsado [FUNC] y pulsa [SETTINGS] primero: salir del modo PERFORM devuelve el pattern
a su último estado guardado (§10.12). Ahora mantén pulsado [FUNC] y pulsa [TRK]: el modo PERFORM está
...
screen: { bank: "A01", tempo: 92, track: 1, type: "SUBTRACKS" }
keys16: { 5: red, 13: red, 14: red, 15: red, 16: red }
hear: El mismo compás con el que empezaste la sesión, y sin dos loops iguales — un kick de pickup una vuelta sí y otra no, la mitad de las ghost notes, un hi-hat que responde solo cuando su pregunta llegó, y un fill bajo tu pulgar.
...
pistas del simulador son exactamente esa pareja. Lo que lee NEI en una subtrack dentro de un
kit, el manual no lo dice.
