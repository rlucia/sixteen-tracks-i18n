...
export const SOURCE_RE = /^(manual §(?:\d+|[A-Z])(?:\.\d+)*(, §(?:\d+|[A-Z])(?:\.\d+)*)*|device|community https?:\/\/\S+)$/;
...
export function parseSession(text, file) {
  ...
  const out = steps.map(s => {
    ...
    return {
      ...
      new: h.new == null ? null : String(h.new), prose: s.prose.join('\n').trim(),
...
export function validateSession(s, ctx) {
  ...
  for (const st of s.steps) {
    ...
    if (!SOURCE_RE.test(st.source)) e.push(`${at}: source "${st.source}" must be "manual §n.n", "device" or "community <url>"`);
